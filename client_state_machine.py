...
class ClientSM:
    ...
    def connect_to(self, peer):
        ...
        if response["status"] == "success":
            ...
            self.out_msg += 'User is currently busy. Please try again later\n'
        elif response["status"] == "self":
            self.out_msg += 'you cannot talk to yourself okay(sick)\n'
        else:
            self.out_msg += 'User is currently not online, try again some other time\n'
        return(False)
    ...
    def disconnect(self):
        ...
        mysend(self.s, msg)
        self.out_msg += 'You have been disconnected from ' + self.peer + '\n'
        self.peer = ''
    ...
    def tictactoe_to(self, peer):
        ...
        if response["status"] == "success":
            ...
            self.out_msg += 'User is currently busy. Please try again some other time\n'
        elif response["status"] == "self":
            self.out_msg += 'You should not play with yourself okay(sick)\n'
        else:
            self.out_msg += 'User is currently not online, try again some other time\n'
        return(False)
    ...
    def player_move(self, command, key):
        if self.valid_move(command) != False:
            ...
            if  self.array[self.move[1]][self.move[0]] == " ":
                ...
                self.out_msg += "This is an invalid move, the position is already taken.\n"  
        else:
            self.out_msg += "This is an invalid move, please add the correct coordinates.\n" 
        ...
        if self.state == S_LOGGEDIN:
            ...
            if len(my_msg) > 0:
                ...
                if my_msg == 'q':
                    ...
                    mode = input("Enter 2:play with your computer:")
                    play_mode = Offline()
                    if mode in ['2']:
                        play_mode.main(mode)
                    else:
...
#==============================================================================
            ...
            if (len(my_msg) > 0 ) and (my_msg[0:9] == 'tictactoe'):
                ...
                if self.tictactoe_to(peer) == True:
                    ...
                    self.out_msg += '-----------------------------------\n'
                    self.out_msg += 'HERE IS HOW IT WORKS:'
...
#==============================================================================
            ...
            if len(my_msg) > 0:         
                ...
                if my_msg == 'quit' :
                    ...
                    if self.draw(self.array):
                        self.print_array
                        self.out_msg += "\nA DRAW"
                        self.out_msg += "\nThe game has ended. Enjoy chatting now.\n\n"
                        self.state = S_CHATTING
                        self.game_peer = ''
                        ...
                        self.out_msg += '\n\nWaiting on other players to make a move... \n\n'
                        self.state = S_WAITING
            ...
            if len(peer_msg) > 0:
                ...
                if peer_msg["action"] == "move":
                    ...
                    self.out_msg += 'Your partner just quit the game. Resuming chat right now...\n'
                    self.game_peer = ''
                    self.state = S_CHATTING
